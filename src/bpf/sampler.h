...
struct saccade_sample {
    ...
    __u64 counters[MAX_COUNTERS]; // Absolute perf counter readings (not deltas); delta computed in
                                  // userspace.
    __u64 events[MAX_COUNTERS];   // active_counter_ids slot values at sample time, identifying each
                                  // counter.
